...
TEST_CASE("test_div")
{
    ...
    std::string s1 = "(3/1)";
    auto expr1 = parser.parse(context,s1.c_str(), s1.c_str()+ s1.length(), &pend, temp_json_values);
    auto result1 = expr1.eval(context, temp_json_values);
    CHECK(json(3) == result1);

    std::string s2 = "(3/@.length)";
    auto expr2 = parser.parse(context,s2.c_str(), s2.c_str()+ s2.length(), &pend, temp_json_values);
    auto result2 = expr2.eval(context, temp_json_values);
    CHECK(json(3) == result2);

    std::string s3 = "(5/2)";
    auto expr3 = parser.parse(context,s3.c_str(), s3.c_str()+ s3.length(), &pend, temp_json_values);
    auto result3 = expr3.eval(context, temp_json_values);
    CHECK(json(2.5) == result3);

    std::string s4 = "(@.length/3)";
    auto expr4 = parser.parse(context,s4.c_str(), s4.c_str()+ s4.length(), &pend, temp_json_values);
    auto result4 = expr4.eval(context, temp_json_values);
    CHECK(0.333333 == Approx(result4.as<double>()).epsilon(0.001));

    std::string s5 = "(@.0/@.length)";
    auto expr5 = parser.parse(context,s5.c_str(), s5.c_str()+ s5.length(), &pend, temp_json_values);
    auto result5 = expr5.eval(context, temp_json_values);
...
TEST_CASE("test_lt")
{
    ...
    auto expr6 = parser.parse(context, s6.c_str(), s6.c_str()+ s6.length(), &pend, temp_json_values);
    auto result6 = expr6.eval(context, temp_json_values);
...
TEST_CASE("test_jsonpath_filter")
{
    ...
    auto res1 = parser.parse(parent, expr1.c_str(), expr1.c_str()+ expr1.length(), &pend, temp_json_values);
    auto result1 = res1.eval(parent, temp_json_values);
    CHECK(json(2) == result1);

    std::string expr2 = "(1 - 1)";
    auto res2 = parser.parse(parent, expr2.c_str(), expr2.c_str()+ expr2.length(), &pend, temp_json_values);
    auto result2 = res2.eval(parent, temp_json_values);
    CHECK(json(0) == result2);

    std::string expr3 = "(@.length - 1)";
    auto res3 = parser.parse(parent, expr3.c_str(), expr3.c_str()+ expr3.length(), &pend, temp_json_values);
    auto result3 = res3.eval(parent, temp_json_values);
...
TEST_CASE("test_jsonpath_filter_negative_numbers")
{
    ...
    auto res4 = parser.parse(parent, expr4.c_str(), expr4.c_str()+ expr4.length(), &pend, temp_json_values);
    auto result4 = res4.eval(parent, temp_json_values);
    CHECK(json(2) == result4);

    std::string expr5 = "((-2 < -1) && (-3 > -4))";
    auto res5 = parser.parse(parent, expr5.c_str(), expr5.c_str()+ expr5.length(), &pend, temp_json_values);
    auto result5 = res5.eval(parent, temp_json_values);
    CHECK(json(true) == result5);

    std::string expr6 = "((-2 < -1) || (-4 > -3))";
    auto res6 = parser.parse(parent, expr6.c_str(), expr6.c_str()+ expr6.length(), &pend, temp_json_values);
    auto result6 = res6.eval(parent, temp_json_values);
    CHECK(json(true) == result6);

    std::string expr7 = "(-2 < -1 && -3 > -4)";
    auto res7 = parser.parse(parent, expr7.c_str(), expr7.c_str()+ expr7.length(), &pend, temp_json_values);
    auto result7 = res7.eval(parent, temp_json_values);
    CHECK(json(true) == result7);

    std::string expr8 = "(-2 < -1 || -4 > -3)";
    auto res8 = parser.parse(parent, expr8.c_str(), expr8.c_str()+ expr8.length(), &pend, temp_json_values);
    auto result8 = res8.eval(parent, temp_json_values);
...
TEST_CASE("test_jsonpath_filter_uni")
{
    ...
    std::string expr1 = "(0)";
    auto res = parser.parse(parent, expr1.c_str(), expr1.c_str()+ expr1.length(), &pend, temp_json_values);
