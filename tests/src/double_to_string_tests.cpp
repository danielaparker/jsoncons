...
std::basic_string<CharT> float_to_string(double val, uint8_t precision)
{
    ...
    jsoncons::detail::string_result<std::basic_string<CharT>> writer(s);
...
TEST_CASE("test_double_to_string")
{
    ...
    CHECK((s == std::string("1.0e+100") || s == std::string("1.0e100")));

    x = 1.0e-100;
    s = float_to_string<char>(x, std::numeric_limits<double>::digits10);
    CHECK(s == std::string("1.0e-100"));
    ...
    CHECK((s == std::string("1.23456e+6") || s == std::string("1.23456e+06") || s == std::string("1.23456e+006") || s == std::string("1234560")));
...
TEST_CASE("test_double_to_wstring")
{
    ...
    CHECK((s == std::wstring(L"1.0e+100") || s == std::wstring(L"1.0e100")));

    x = 1.0e-100;
    s = float_to_string<wchar_t>(x, std::numeric_limits<double>::digits10);
    CHECK(s == std::wstring(L"1.0e-100"));
