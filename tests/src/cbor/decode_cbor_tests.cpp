...
TEST_CASE("dump cbor to string test")
{
    ...
    bignum n = bignum::from_bytes_be(sign_t::plus, bytes.data(), bytes.size());
    n = -1 - n;
    std::string s;
    n.dump(s);
...
TEST_CASE("cbor conversion tests")
{
    ...
    it2++;
    CHECK(bool(it2->as<bignum>() == bignum::from_string("-18446744073709551617")));
    it2++;
...
*/
    ...
    SECTION("j[0].as<T>()")
    {
        ...
        CHECK(bool(j[2].as<jsoncons::bignum>() == jsoncons::bignum::from_string("-18446744073709551617")));
