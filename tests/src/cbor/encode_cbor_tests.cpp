...
TEST_CASE("cbor_arrays_and_maps")
{
    ...
    // big float
    json j(json::array({-2,27315}),semantic_tag::big_float);
    CHECK(j.get_semantic_tag() == semantic_tag::big_float);
    json j2 = j;
    CHECK(j2.get_semantic_tag() == semantic_tag::big_float);
    json j3;
    j3 = j;
    CHECK(j3.get_semantic_tag() == semantic_tag::big_float);
