...
private:
    ...
    virtual void do_string_value(const Char* p, size_t length, basic_parse_context<Char>)
    {
        if (stack_.back().is_object())
        {
    ...
    virtual void do_longlong_value(long long value, basic_parse_context<Char>)
    {
        if (stack_.back().is_object())
        {
    ...
    virtual void do_ulonglong_value(unsigned long long value, basic_parse_context<Char>)
    {
        if (stack_.back().is_object())
        {
            stack_.back().object_->push_back(std::move(stack_.back().name_),basic_json<Char,Alloc>(value));
    ...
    virtual void do_double_value(double value, basic_parse_context<Char>)
    {
        if (stack_.back().is_object())
        {
            stack_.back().object_->push_back(std::move(stack_.back().name_),basic_json<Char,Alloc>(value));
    ...
    virtual void do_bool_value(bool value, basic_parse_context<Char>)
    {
        if (stack_.back().is_object())
        {
            stack_.back().object_->push_back(std::move(stack_.back().name_),basic_json<Char,Alloc>(value));
