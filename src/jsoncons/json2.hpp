...
{
    return proxy<size_t>(var_.get(key_),i);
}
...
{
    return proxy<std::basic_string<Char>>(var_.get(key_),name);
}
...
{
    return var_.get(key_).to_string();
}
...
{
    return var_->set_member(name,value);
}
