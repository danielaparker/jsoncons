...
Output name event for a name-value pair. Uses `do_name`.
    ...
    void name(const Char* p, size_t length, const basic_parse_context<Char>& context)
Output name event for a name-value pair. Uses `do_name`.

    void value(const std::string& value)
Output character data event. Uses `do_string_value`.

    void value(const Char* p, size_t length, const basic_parse_context<Char>& context)
Output character data event. Uses `do_string_value`.
