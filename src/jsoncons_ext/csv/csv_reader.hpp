...
void basic_csv_reader<Char,Alloc>::read_array_of_arrays()
{
    ...
    while (!eof())
    {
        ...
        else if (column_ == 1 && c == comment_symbol_)
        {
            ...
            if (eof())
            {
                err_handler_->fatal_error(jsoncons::json_parser_error::unexpected_eof, "Unexpected EOF", *this);
            }
...
void basic_csv_reader<Char,Alloc>::parse_quoted_string()
{
    ...
    while (!done_string)
    {
        ...
        if (eof())
        {
            err_handler_->fatal_error(jsoncons::json_parser_error::unexpected_eof, "EOF, expected quote character", *this);
        }
