    ...
    */
    ...
    basic_csv_reader(std::basic_istream<Char>& is,
       ...
       :
         ...
         err_handler_(std::addressof(jsoncons::default_input_error_handler<Char>())),
         assume_header_(),
         field_delimiter_(),
         quote_char_(),
         quote_escape_char_(),
...
void basic_csv_reader<Char,Alloc>::read_array_of_arrays()
{
    ...
    while (!eof())
    {
        ...
        else if (column_ == 1 && c == comment_symbol_)
        {
            ...
            if (eof())
            {
                err_handler_->error(std::error_code(jsoncons::json_parser_error::unexpected_eof, jsoncons::json_parser_category()), *this);
            }
...
void basic_csv_reader<Char,Alloc>::parse_quoted_string()
{
    ...
    while (!done_string)
    {
        ...
        if (eof())
        {
            err_handler_->error(std::error_code(jsoncons::json_parser_error::eof_reading_string_value, jsoncons::json_parser_category()), *this);
        }
