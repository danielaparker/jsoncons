...
class my_json_visitor : public default_json_visitor
{
    ...
    JSONCONS_VISITOR_RET_TYP visit_typed_array(const span<const uint16_t>& s, 
                                semantic_tag tag, 
                                const ser_context&, 
    ...
    JSONCONS_VISITOR_RET_TYP visit_typed_array(half_arg_t, const span<const uint16_t>& s,
        semantic_tag tag,
        const ser_context&,
        std::error_code&) override
    {
