...
TEST_CASE("test cbor encode with raw tags")
{
    SECTION("test 1")
    {
        ...
        CHECK(jsoncons::staj_event_flags::begin_array == cursor.current().event_type());
        cursor.next();
        CHECK(0xC1 == cursor.raw_tag());
        CHECK(jsoncons::staj_event_flags::null_value == cursor.current().event_type());
        cursor.next();
        ...
        CHECK(jsoncons::staj_event_flags::begin_object == cursor.current().event_type());
    }
}
