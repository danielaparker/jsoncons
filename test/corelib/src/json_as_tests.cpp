...
TEST_CASE("json::as<jsoncons::bigint>()")
{
    ...
    SECTION("from bigint")
    {
        ...
        CHECK(bool(doc.as<jsoncons::bigint>() == jsoncons::bigint(s)));
    }
}
...
TEST_CASE("json::as<__int128>()")
{
    ...
    __int128 n{};
    auto result = jsoncons::dec_to_integer(s1.data(),s1.size(), n);
    ...
    std::string s2;
    jsoncons::from_integer(val, s2);

    std::string s3;
    jsoncons::from_integer(n, s3);
