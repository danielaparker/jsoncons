...
TEST_CASE("json_ref array tests")
{
    ...
    {
        json other{ j };
        json j1(json_ptr_arg, &other);
        json j2(const_json_ptr_arg, &other);
...
TEST_CASE("json_ref bool tests")
{
    ...
    SECTION("true")
    {
        json v(json_ptr_arg, &tru);
        REQUIRE(v.is_bool());
        CHECK(v.as_bool());
    }
    SECTION("false")
    {
        json v(json_ptr_arg, &fal);
        REQUIRE(v.is_bool());
