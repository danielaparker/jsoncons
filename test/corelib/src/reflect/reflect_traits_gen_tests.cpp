...
JSONCONS_N_MEMBER_TRAITS(ns::Item, 1, name, label)
JSONCONS_N_MEMBER_TRAITS(ns::Container, 0, items)
        ...
        std::string input = R"(
{
    ...
    "title" : "Kafka on the Shore"    
        ...
        REQUIRE_FALSE(result);
        CHECK(result.error().code() == jsoncons::conv_errc::missing_required_member);
        CHECK("ns::book_all_cg::price" == result.error().msg_arg());
        //std::cout << result.error() .message() << "\n";
    }
    SECTION("invalid JSON value")
    {
        ...
        std::string input = R"(
{
    ...
    "Title" : "Kafka on the Shore"    
        ...
        REQUIRE_FALSE(result);
        CHECK(result.error().code() == jsoncons::conv_errc::missing_required_member);
        CHECK("ns::book_all_gs::Price" == result.error().msg_arg());
        //std::cout << result.error() .message() << "\n";
    }
    SECTION("invalid JSON value")
    {
...
TEST_CASE("JSONCONS_N_MEMBER_TRAITS with mandatory-field errors when nested inside an optional parent member tests")
{
