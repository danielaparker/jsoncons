...
TEST_CASE("decoder primitive")
{
    ...
    SECTION("uint64_t")
    {
        ...
        auto result = reflect::decoder<uint64_t>::try_decode(make_alloc_set(), cursor);
        REQUIRE(result);
    ...
    SECTION("vector of uint64_t")
    {
        ...
        json_string_cursor cursor(input);
        auto result = reflect::decoder<std::vector<uint64_t>>::try_decode(make_alloc_set(), cursor);
        REQUIRE(result);
...
TEST_CASE("decoder std::string")
{
    ...
    SECTION("string")
    {
        ...
        json_string_cursor cursor(input);
        auto result = reflect::decoder<std::string>::try_decode(make_alloc_set(), cursor);
        REQUIRE(result);
...
TEST_CASE("decoder std::pair")
{
    SECTION("std::pair<std::string,std::string>")
    {
        ...
        json_string_cursor cursor(input);
        auto result = reflect::decoder<value_type>::try_decode(make_alloc_set(), cursor);
        REQUIRE(result);
...
TEST_CASE("decoder deserialization errors")
{
