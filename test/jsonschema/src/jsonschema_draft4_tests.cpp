        ...
        for (const auto& test_group : tests.array_range()) 
        {
            ...
            try
            {
                jsonschema::json_schema<json> compiled = jsonschema::make_json_schema(test_group.at("schema"), resolver, 
                    jsonschema::evaluation_options{}.default_version(jsonschema::schema_dialect::draft4())
...
TEST_CASE("jsonschema draft4 tests")
{
    ...
    SECTION("tests")
    {
        ...
        //jsonschema_tests("./jsonschema/JSON-Schema-Test-Suite/tests/draft4/optional/format/json-pointer.json");
