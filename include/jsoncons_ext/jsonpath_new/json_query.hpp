...
namespace jsoncons { namespace jsonpath_new {
    ...
    struct state_item
    {
        ...
        state_item()
            : state(path_state::start), is_union(false)
        {
        }

        explicit state_item(path_state state) noexcept
            : state(state), is_union(false)
        {
        }
    ...
    private:
        ...
        public:
            ...
            token(begin_multi_select_list_arg_t) noexcept
                : type_(path_token_type::begin_multi_select_list)
            {
            }

            token(end_multi_select_list_arg_t) noexcept
                : type_(path_token_type::end_multi_select_list)
            {
            }
            ...
            token(std::unique_ptr<selector_base>&& expression)
    ...
    public:
        ...
        public:
            ...
            jsonpath_expression(jsonpath_expression&& expr)
                : tokens_(std::move(expr.tokens_))
            {
            }

            jsonpath_expression(std::vector<token>&& tokens)
            ...
            Json evaluate(jsonpath_resources<Json>& dynamic_resources, reference instance)
            {
                ...
                if (!tokens_.empty())
                {
                         ...
                         )
                    {
                        ...
                        else if (tokens_[i].is_recursive_descent())
                        {
            ...
            static jsonpath_expression compile(jsonpath_resources<Json>& resources, const string_view_type& expr)
            {
                jsoncons::jsonpath_new::detail::jsonpath_evaluator<Json,const Json&,detail::VoidPathConstructor<Json>> evaluator;
                std::error_code ec;
                auto tokens = evaluator.compile(resources, expr.data(), expr.size(), ec);
                if (ec)
                {
                    JSONCONS_THROW(jsonpath_error(ec, evaluator.line(), evaluator.column()));
                }
                return jsonpath_expression(std::move(tokens));
            }
    ...
    public:
            ...
            while (p_ < end_input_)
            {
                switch (state_stack_.back().state)
                {
                    ...
                    case path_state::path_or_function_name:
                        switch (*p_)
                        {
                            ...
                            case '[':
                            {
                                ...
                                state_stack_.back().state = path_state::bracket_specifier_or_union;
                                ++p_;
                                ++column_;
                                break;
                            }
                            ...
                            case '*':
                            {
                                push_token(token(jsoncons::make_unique<wildcard_selector>()));
                                state_stack_.back().state = path_state::dot;
                                ++p_;
                                ++column_;
                                break;
                    ...
                    case path_state::dot_or_left_bracket: 
                        switch (*p_)
                        {
                            ...
                            case '.':
                                ...
                                break;
                            case '[':
                                state_stack_.emplace_back(path_state::bracket_specifier_or_union, state_stack_.back());
                                ++p_;
                                ++column_;
                                break;
                            default:
                    ...
                    case path_state::bracket_specifier_or_union:
                        switch (*p_)
                        {
                            ...
                            case '?':
                            {
                                ...
                                line_ = parser.line();
                                column_ = parser.column();
                                push_token(token(jsoncons::make_unique<filter_selector>(result)));
                                state_stack_.back().state = path_state::comma_or_right_bracket;
                                break;                   
                            }
                            case ':': // slice_expression
                                ...
                                break;
                            case '*':
                                state_stack_.back().state = path_state::bracketed_wildcard_or_union;
                                ++p_;
                                ++column_;
                                break;
                            case '\'':
                            ...
                            case '-':case '0':case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8':case '9':
                                state_stack_.back().state = path_state::index_or_slice_expression;
                                state_stack_.emplace_back(path_state::number);
                                break;
                            default:
                                buffer.clear();
                    ...
                    case path_state::rhs_slice_expression_start:
                    {
                        ...
                        {
                            case ']':
                                push_token(token(jsoncons::make_unique<slice_selector>(slic)));
                                slic = slice{};
                    ...
                    case path_state::bracketed_name_or_path:
                        ...
                        break;
                    case path_state::bracketed_wildcard_or_union:
                        switch (*p_)
                        {
                            case ' ':case '\t':case '\r':case '\n':
                                advance_past_space_character();
                            ...
                            case '.':
                                ...
                                buffer.push_back(*p_);
                                state_stack_.back().state = path_state::comma_or_right_bracket;
                                state_stack_.emplace_back(path_state::path, state_stack_.back());
                                ++p_;
                                ++column_;
                                break;
                            case '[':
                                ...
                                buffer.push_back(*p_);
                                state_stack_.back().state = path_state::comma_or_right_bracket;
                                state_stack_.emplace_back(path_state::path2, state_stack_.back());
                                ++p_;
                                ++column_;
                                break;
                            case ',':
        ...
        void push_token(token&& tok)
        {
            switch (tok.type())
            {
                case path_token_type::selector:
                {
                    if (!tokens_.empty() && tokens_.back().is_projection() && 
                        (tok.precedence_level() < tokens_.back().precedence_level() ||
                        (tok.precedence_level() == tokens_.back().precedence_level() && tok.is_right_associative())))
                    {
                        tokens_.back().selector_->add_selector(std::move(tok.selector_));
                    }
                    else
                    {
                        tokens_.emplace_back(std::move(tok));
                    }
                    break;
                }
