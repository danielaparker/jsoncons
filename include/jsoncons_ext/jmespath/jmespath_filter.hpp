...
#ifndef JSONCONS_JMESPATH_JMESPATH_FILTER_HPP
#define JSONCONS_JMESPATH_JMESPATH_FILTER_HPP
...
namespace jsoncons { namespace jmespath { namespace detail {
...
public:
    ...
    {
        JSONCONS_THROW(jmespath_error(jmespath_errc::invalid_filter_unsupported_operator));
    }
...
public:
    ...
    {
        return jsoncons::jmespath::detail::unary_minus(value_);
    }
};
...
public:
    ...
    {
        jmespath_evaluator<Json,const Json&,VoidPathConstructor<Json>> evaluator(line_,column_);
    ...
    Json unary_minus() const override
    {
        return nodes_.size() == 1 ? jsoncons::jmespath::detail::unary_minus(nodes_[0]) : Json::null();
    }
};
...
template <class Json>
class jmespath_filter_expr
{
public:
    std::vector<token<Json>> tokens_;
public:
    jmespath_filter_expr()
    {
    }

    jmespath_filter_expr(const std::vector<token<Json>>& tokens)
        : tokens_(tokens)
    {
    }
...
public:
    jmespath_filter_parser()
        : line_(1), column_(1)
    {
    }
    jmespath_filter_parser(std::size_t line, std::size_t column)
    ...
    jmespath_filter_expr<Json> parse(const Json& root, const char_type* p, const char_type* end_expr, const char_type** end_ptr)
    {
        output_stack_.clear();
            ...
            switch (state)
            {
                ...
                case filter_state::expect_more_args_or_right_round_bracket:
                {
                    switch (*p)
                    {
                        ...
                        default:
                            JSONCONS_THROW(jmespath_error(jmespath_errc::invalid_filter_unsupported_operator, line_, column_));
                    }
                    ++p;
                    ++column_;
                    break;
                    ...
                    switch (*p)
                    {
                        case ' ':case '\t':
                            if (buffer.length() > 0)
                            {
                                ...
                                {
                                    JSONCONS_THROW(jmespath_error(jmespath_errc::parse_error_in_filter,line_,column_));
                                }
                                buffer.clear();
                                buffer_line = buffer_column = 1;
                            }
            ...
            case filter_state::expect_oper_or_right_round_bracket: 
                switch (*p)
                {
                    ...
                    case '/':
                        ...
                        break;
                    default: 
                        JSONCONS_THROW(jmespath_error(jmespath_errc::invalid_filter,line_,column_));
                        break;
                };
                break;
                    ...
                    case '/':
                        {
                            if (!path_mode_stack_.empty())
                            {
                                ...
                                {
                                    jmespath_evaluator<Json,const Json&,detail::VoidPathConstructor<Json>> evaluator(buffer_line,buffer_column);
                ...
                case filter_state::expect_regex: 
                    switch (*p)
                    {
                        ...
                        case '/':
                            ...
                            break;
                        default: 
                            JSONCONS_THROW(jmespath_error(jmespath_errc::invalid_filter_expected_slash,line_,column_));
                            break;
                    };
                    ++p;
                    ++column_;
        ...
        if (depth != 0)
        {
            JSONCONS_THROW(jmespath_error(jmespath_errc::invalid_filter_unbalanced_paren,line_,column_));
        }
        *end_ptr = p;

        return jmespath_filter_expr<Json>(output_stack_);
    }
};
