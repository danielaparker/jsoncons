...
class jmespath_evaluator : public ser_context
{
    ...
    class list_projection final : public evaluator_base
    {
    public:
        list_projection()
        {
    ...
    private:
        ...
        std::vector<std::unique_ptr<selector_base>> rhs_selectors_;
    public:
        slice_projection_old(const slice& a_slice)
            : slice_(a_slice)
        {
        }
    ...
    class slice_projection final : public evaluator_base
    {
    private:
        slice slice_;
    public:
    ...
    public:
        ...
        filter_expression_old(std::unique_ptr<selector_base>&& cmp_selector)
            : cmp_selector_(std::move(cmp_selector))
        {
        }
    ...
    class comparator_evaluator final : public evaluator_base
    {
    public:
        Comparator cmp_;
        ...
        reference evaluate(jmespath_context& context, reference val, std::error_code& ec) override
        {
            ...
            reference rhs = this->apply_evaluators(context, val, ec);
            if (ec)
            {
                return Json::null();
            }
        ...
        string_type to_string() const override
        {
            return string_type("comparator_evaluator\n");
        }
    };
    ...
    std::vector<key_selector_old> key_evaluator_stack_old_;
...
public:
    jmespath_evaluator()
        : line_(1), column_(1),
          ...
          p_(nullptr)
    {
        key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<compound_expression>());
    }
                       ...
                       std::error_code& ec)
    {
        ...
        while (p_ < end_input_)
        {
            switch (state_stack_.back())
            {
                ...
                case path_state::expression: 
                {
                    switch (*p_)
                    {
                        ...
                        case '*':
                            ...
                            break;
                        case '&':
                            key_evaluator_stack_old_.back().selector->expression_type(true);
                            ++p_;
                            ++column_;
                            break;                        
                        default:
                ...
                case path_state::key_expr:
                    key_evaluator_stack_old_.back().key = std::move(buffer);
                    buffer.clear(); 
                    state_stack_.pop_back(); 
                    break;
                case path_state::val_expr:
                    switch(*p_)
                    {
                        case '\"':
                            ...
                            key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<identifier_selector_old>(buffer));
                            buffer.clear();
                            state_stack_.pop_back(); 
                            break;
                        default:
                ...
                case path_state::identifier_or_function_expr:
                    switch(*p_)
                    {
                        case '(':
                        {
                            ...
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<function_selector>(it->second));
                            structure_offset_stack_.push_back(key_evaluator_stack_old_.size());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<compound_expression>());
                ...
                case path_state::arg_or_right_paren:
                    switch (*p_)
                    {
                        ...
                        case ')':
                        {
                            ...
                            // new
                            auto& selector2 = key_evaluator_stack_old_[pos-1].selector;
                            for (size_t i = pos; i < key_evaluator_stack_old_.size(); ++i)
                            {
                                selector2->add_selector(std::move(key_evaluator_stack_old_[i].selector));
                            }
                            key_evaluator_stack_old_.erase(key_evaluator_stack_old_.begin()+pos, key_evaluator_stack_old_.end());
                ...
                case path_state::raw_string: 
                    switch (*p_)
                    {
                        case '\'':
                        {
                            key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<json_value_selector>(Json(buffer)));
                            buffer.clear();
                ...
                case path_state::json_value: 
                    switch (*p_)
                    {
                        case '`':
                        {
                            ...
                            key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<json_value_selector>(std::move(j)));
                            buffer.clear();
                ...
                case path_state::compound_expression:
                    switch(*p_)
                    {
                        ...
                        case '|':
                        {
                            ++p_;
                            ++column_;
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<pipe_selector>());
                            state_stack_.emplace_back(path_state::expression);
                            break;
                        }
                        case '[':
                ...
                case path_state::bracket_specifier:
                    ...
                    {
                        case '*':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<list_projection>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<list_projection_old>());
                    ...
                    switch(*p_)
                    {
                        case ']':
                        {
                            ...
                            else
                            {
                                ...
                                key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<index_selector_old>(r.value()));
                                buffer.clear();
                            }
                            state_stack_.pop_back(); // bracket_specifier
                            ++p_;
                ...
                case path_state::bracket_specifier2:
                {
                    ...
                    {
                        case ']':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<slice_projection>(a_slice));
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<slice_projection_old>(a_slice));
                            a_slice = slice{};
                ...
                case path_state::cmp_lt_or_lte:
                {
                    ...
                    {
                        case '=':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_lte>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_lte>>());
                            ...
                            break;
                        default:
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_lt>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_lt>>());
                            state_stack_.back() = path_state::expect_filter_right_bracket;
                            state_stack_.emplace_back(path_state::expression);
                            break;
                    }
                ...
                case path_state::cmp_eq:
                {
                    ...
                    {
                        case '=':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_eq>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_eq>>());
                            state_stack_.back() = path_state::expect_filter_right_bracket;
                            state_stack_.emplace_back(path_state::expression);
                            ++p_;
                            ++column_;
                ...
                case path_state::cmp_gt_or_gte:
                {
                    ...
                    {
                        case '=':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_gte>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_gte>>());
                            state_stack_.back() = path_state::expect_filter_right_bracket;
                            state_stack_.emplace_back(path_state::expression);
                            ++p_;
                            ++column_;
                            break;
                        default:
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_gt>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_gt>>());
                            state_stack_.back() = path_state::expect_filter_right_bracket;
                            state_stack_.emplace_back(path_state::expression);
                            break;
                    }
                ...
                case path_state::cmp_ne:
                {
                    ...
                    {
                        case '=':
                            key_evaluator_stack_.emplace_back(jsoncons::make_unique<comparator_evaluator<cmp_ne>>());
                            key_evaluator_stack_old_.emplace_back(jsoncons::make_unique<comparator_selector_old<cmp_ne>>());
                            state_stack_.back() = path_state::expect_filter_right_bracket;
                            state_stack_.emplace_back(path_state::expression);
                            ++p_;
                            ++column_;
                ...
                case path_state::expect_filter_right_bracket:
                {
                    switch(*p_)
                    {
                        ...
                        case ']':
                        {
                            ...
                            // old
                            auto pp = std::move(key_evaluator_stack_old_[pos]);
                            for (size_t i = pos+1; i < key_evaluator_stack_old_.size(); ++i)
                            {
                                pp.selector->add_selector(std::move(key_evaluator_stack_old_[i].selector));
                            }
                            key_evaluator_stack_old_.erase(key_evaluator_stack_old_.begin()+pos, key_evaluator_stack_old_.end());
                            auto qq = jsoncons::make_unique<filter_expression_old>(std::move(pp.selector));
                            key_evaluator_stack_old_.back() = key_selector_old(std::move(qq));                            
                ...
                case path_state::expect_right_bracket4:
                {
                    switch(*p_)
                    {
                        ...
                        case ']':
                        {
                            ...
                            selectors2.reserve(key_evaluator_stack_old_.size()-pos);
                            for (size_t i = pos; i < key_evaluator_stack_old_.size(); ++i)
                            {
                                selectors2.emplace_back(std::move(key_evaluator_stack_old_[i].selector));
                            }
                            key_evaluator_stack_old_.erase(key_evaluator_stack_old_.begin()+pos, key_evaluator_stack_old_.end());
                            key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<multi_select_list>(std::move(selectors2)));
                ...
                case path_state::expect_right_brace:
                {
                    switch(*p_)
                    {
                        ...
                        case '}':
                        {
                            ...
                            key_selectors2.reserve(key_evaluator_stack_old_.size()-pos);
                            for (size_t i = pos; i < key_evaluator_stack_old_.size(); ++i)
                            {
                                key_selectors2.emplace_back(std::move(key_evaluator_stack_old_[i]));
                            }
                            key_evaluator_stack_old_.erase(key_evaluator_stack_old_.begin()+pos, key_evaluator_stack_old_.end());
                            key_evaluator_stack_old_.back().selector->add_selector(jsoncons::make_unique<multi_select_hash>(std::move(key_selectors2)));
        ...
        //for (auto& item : key_evaluator_stack_old_)
        //{
    ...
    reference evaluate_old(reference root, std::error_code& ec)
    {
        ...
        for (auto& t : key_evaluator_stack_old_)
        {
