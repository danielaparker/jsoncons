...
/*
    ...
    public:
        ...
        reference select(jmespath_context& context, 
                         ...
                         std::error_code& ec) override
        {
            std::cout << "(list_projectionB2) " << pretty_print(val) << "\n";
...
*/
    ...
    class identifier_selector2 final : public evaluator_base
    {
    private:
        string_type identifier_;
    public:
        identifier_selector2(const string_view_type& name)
            : identifier_(name)
        {
        }

        reference select(jmespath_context&, 
                         reference& value,
                         std::error_code&) override
        {
            std::cout << "(identifier_evaluator " << identifier_  << ")" << "\n";

            std::cout << pretty_print(value) << "\n\n";

            if (value.is_object() && value.contains(identifier_))
            {
                return value.at(identifier_);
            }
            else 
            {
                return Json::null();
...
*/
    ...
    public:
        ...
        token(pointer value)
            : type_(token_type::value), value_(value)
        {
        }
                         ...
                         std::error_code& ec)
        {
            ...
            {
                case token_type::evaluator:
                    return evaluator_->select(context, value, ec);
                default:
                    return Json::null();
            }
        }
...
public:
                ...
                case path_state::val_expr:
                    switch(*p_)
                    {
                        case '\"':
                            ++p_;
                            ++column_;
                            push_token(token(jsoncons::make_unique<identifier_selector2>(buffer)));
    ...
    reference evaluate(reference root, std::error_code& ec)
    {
        ...
        std::cout << "root: " << pretty_print(root) << "\n\n";
        ...
        for (auto&& t : output_stack_)
        {
            ...
            else if (t.is_evaluator())
            {
                ...
                stack.pop_back();
                ptr = std::addressof(t.select(temp_factory_, *ptr, ec));
                stack.push_back(ptr);
            }
        }
        if (stack.size() != 1)
