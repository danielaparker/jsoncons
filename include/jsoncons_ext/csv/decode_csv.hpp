...
namespace csv {
    ...
    typename std::enable_if<is_basic_json<T>::value,T>::type
    decode_csv(Iterator first, Iterator last,
                const basic_csv_decode_options<typename std::iterator_traits<Iterator>::value_type>& options = 
                    basic_csv_decode_options<typename std::iterator_traits<Iterator>::value_type>())
    {
        using char_type = typename std::iterator_traits<Iterator>::value_type;

        jsoncons::json_decoder<T> decoder;
        basic_csv_reader<char_type, iterator_source<Iterator>> reader(iterator_source<Iterator>(first,last), decoder, options);
        reader.read();
        return decoder.get_result();
    }
                    ...
                    basic_csv_decode_options<typename std::iterator_traits<Iterator>::value_type>())
    {
        ...
        basic_csv_cursor<char_type,iterator_source<Iterator>> cursor(iterator_source<Iterator>(first, last), options);
