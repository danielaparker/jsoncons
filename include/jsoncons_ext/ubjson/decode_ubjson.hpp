...
namespace ubjson {
    ...
    typename std::enable_if<extension_traits::is_basic_json<T>::value &&
                            extension_traits::is_byte_sequence<Source>::value,T>::type 
    decode_ubjson(const wrapped_allocators<ResultAllocator,WorkAllocator>& allocators,
                  const Source& v, 
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        json_decoder<T,WorkAllocator> decoder(allocators.get_work_allocator());
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_ubjson_reader<jsoncons::bytes_source,WorkAllocator> reader(v, adaptor, options, allocators.get_work_allocator());
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_ubjson(const wrapped_allocators<ResultAllocator,WorkAllocator>& allocators,
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        basic_ubjson_cursor<bytes_source,WorkAllocator> cursor(v, options, allocators.get_work_allocator());
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        json_decoder<T,WorkAllocator> decoder(allocators.get_work_allocator());
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_ubjson_reader<jsoncons::binary_stream_source,WorkAllocator> reader(is, adaptor, options, allocators.get_work_allocator());
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_ubjson(const wrapped_allocators<ResultAllocator,WorkAllocator>& allocators,
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        basic_ubjson_cursor<binary_stream_source,WorkAllocator> cursor(is, options, allocators.get_work_allocator());
        json_decoder<basic_json<char,sorted_policy,WorkAllocator>,WorkAllocator> decoder(allocators.get_work_allocator(), allocators.get_work_allocator());
