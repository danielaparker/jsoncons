...
#include <jsoncons_ext/ubjson/ubjson_error_category.hpp>

namespace jsoncons { namespace ubjson {

class ubjson_parser : public serializing_context
{
    ...
    std::string buffer_;
public:
    ubjson_parser(json_content_handler& handler)
       : handler_(handler), 
         column_(1),
         nesting_depth_(0)
    {
        ...
        else
        {
            switch (*pos)
            {
                ...
                case ubjson_format::float32_cd: 
                {
                    ...
                    if (endp == input_ptr_)
                    {
                        JSONCONS_THROW(ubjson_decode_error(end_input_-input_ptr_));
                    }
                    else
                    {
                        input_ptr_ = endp;
                ...
                default:
                {
                    JSONCONS_THROW(ubjson_decode_error(end_input_-pos));
                }
            }
        }
    }
