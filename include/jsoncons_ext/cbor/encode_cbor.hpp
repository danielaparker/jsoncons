...
namespace cbor {
    ...
    typename std::enable_if<ext_traits::is_basic_json<T>::value &&
                            ext_traits::is_back_insertable_byte_container<ByteContainer>::value,void>::type 
    encode_cbor(const allocator_set<Alloc,TempAlloc>& alloc_set,
                const T& j, 
                ByteContainer& cont, 
                const cbor_encode_options& options = cbor_encode_options())
    {
        using char_type = typename T::char_type;
        basic_cbor_encoder<bytes_sink<ByteContainer>,TempAlloc> encoder(cont, options, alloc_set.get_temp_allocator());
        auto adaptor = make_json_visitor_adaptor<basic_json_visitor<char_type>>(encoder);
        j.dump(adaptor);
    }
    ...
    encode_cbor(const allocator_set<Alloc,TempAlloc>& alloc_set,
                ...
                const cbor_encode_options& options = cbor_encode_options())
    {
        basic_cbor_encoder<jsoncons::bytes_sink<ByteContainer>,TempAlloc> encoder(cont, options, alloc_set.get_temp_allocator());
        std::error_code ec;
        reflect::encode_traits<T>::try_encode(val, encoder, ec);
        if (JSONCONS_UNLIKELY(ec))
        {
    ...
    {
        using char_type = typename T::char_type;
        basic_cbor_encoder<binary_stream_sink,TempAlloc> encoder(os, options, alloc_set.get_temp_allocator());
        auto adaptor = make_json_visitor_adaptor<basic_json_visitor<char_type>>(encoder);
        j.dump(adaptor);
    }
