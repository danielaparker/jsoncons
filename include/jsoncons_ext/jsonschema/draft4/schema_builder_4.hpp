...
namespace draft4 {
    ...
    public:
        schema_builder_4(const Json& sch, const schema_builder_factory_type& builder_factory, 
            evaluation_options options, schema_store_type* schema_store_ptr,
            const std::vector<schema_resolver<Json>>& resolvers) 
            : schema_builder<Json>(schema_version::draft4(), sch, builder_factory, options, schema_store_ptr, resolvers)
        {
            init();
        }
