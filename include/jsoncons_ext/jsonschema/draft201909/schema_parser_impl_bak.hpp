            ...
            switch (sch.type())
            {
                case json_type::bool_value:
                {
                    if (sch.template as<bool>())
                    {
                        ...
                        schema_validator_ptr = jsoncons::make_unique<boolean_schema_validator<Json>>(eval_context.eval_path(), schema_path, true);
                    }
                    else
                    {
                        std::string schema_path = new_context.make_schema_path_with("false");
                        schema_validator_ptr = jsoncons::make_unique<boolean_schema_validator<Json>>(eval_context.eval_path(), schema_path, false);
                    }
            ...
            const compilation_context& context, const Json& sch)
        {
            ...
            if (it != sch.object_range().end()) 
            {
                ...
                for (const auto& def : it->value().object_range())
                {
                    ...
                    subschemas_.emplace_back(make_schema_validator(evaluation_context{}, context, def.value(), sub_keys));
                    is_def.pop();
                }
            }
