    ...
    public:
            ...
            const compilation_context& context, const Json& sch)
        {
            ...
            if (it != sch.object_range().end()) // this schema has a reference
            {
                ...
                validators.push_back(jsoncons::make_unique<recursive_ref_validator_type>(eval_context.eval_path(), id.base()));
            }
            ...
            if (it != sch.object_range().end()) 
            {
                validators.emplace_back(make_dependent_required_validator(evaluation_context(eval_context, "dependentRequired"),
                    context, it->value()));
            }
            ...
            if (it != sch.object_range().end()) 
            {
                validators.emplace_back(make_dependent_schemas_validator(evaluation_context(eval_context, "dependentSchemas"),
                    context, it->value()));
            }
            ...
            if (it != sch.object_range().end()) 
            {
                validators.push_back(make_unevaluated_properties_validator(evaluation_context{ eval_context, "unevaluatedProperties" },
                    context, it->value()));
            } 
        ...
        std::unique_ptr<dependent_required_validator<Json>> make_dependent_required_validator(const evaluation_context& eval_context, 
            const compilation_context& context, const Json& sch)
        {
            uri schema_path = context.get_absolute_uri();
            std::map<std::string, keyword_validator_type> dependent_required;
            ...
            for (const auto& dep : sch.object_range())
            {
                switch (dep.value().type()) 
                {
                    case json_type::array_value:
                    {
                        ...
                        dependent_required.emplace(dep.key(), 
                            make_required_validator(evaluation_context{eval_context, "dependentRequired"}, 
                                compilation_context(nullptr, std::vector<schema_location>{{location}}),
                                dep.value().template as<std::vector<std::string>>()));
                        break;
                    }
                    default:
            ...
            return jsoncons::make_unique<dependent_required_validator<Json>>(eval_context.eval_path(), std::move(schema_path),
                std::move(dependent_required));
        }

        std::unique_ptr<dependent_schemas_validator<Json>> make_dependent_schemas_validator(const evaluation_context& eval_context, const compilation_context& context, 
            const Json& sch)
        {
            uri schema_path = context.get_absolute_uri();
            std::map<std::string, schema_validator_type> dependent_schemas;
            ...
            for (const auto& dep : sch.object_range())
            {
                switch (dep.value().type()) 
                {
                    case json_type::object_value:
                    {
                        ...
                        dependent_schemas.emplace(dep.key(),
                            make_schema_validator(evaluation_context{eval_context, "dependentSchemas"}, context, dep.value(), sub_keys));
                        break;
                    }
                    default:
                    {
            ...
            return jsoncons::make_unique<dependent_schemas_validator<Json>>(eval_context.eval_path(), std::move(schema_path),
        ...
        std::unique_ptr<unevaluated_properties_validator<Json>> make_unevaluated_properties_validator(const evaluation_context& eval_context,
            const compilation_context& context, const Json& sch)
        {
            uri schema_path = context.get_absolute_uri();
            ...
            return jsoncons::make_unique<unevaluated_properties_validator<Json>>(eval_context.eval_path(), std::move(schema_path),
                make_schema_validator(eval_context, context, sch, sub_keys));
        }
        ...
        keyword_validator_type get_or_create_reference(const evaluation_context& eval_context, const schema_location& uri)
        {
            ...
            if (uri.has_fragment() && !uri.has_plain_name_fragment()) 
            {
                ...
                if (unprocessed_keywords_it != file.unknown_keywords.end()) 
                {
                    ...
                    auto orig = jsoncons::make_unique<ref_validator_type>(eval_context.eval_path(), uri.base(), std::move(s));
                    auto p = orig.get();
