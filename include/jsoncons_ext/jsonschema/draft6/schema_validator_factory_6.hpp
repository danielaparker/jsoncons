...
namespace draft6 {
    ...
    class schema_validator_factory_6 : public keyword_validator_factory<Json> 
    {
    public:
    ...
    public:
        schema_validator_factory_6(Json&& sch, const validator_factory_factory_type& factory_factory, 
            ...
            : keyword_validator_factory<Json>(schema_version::draft6(), std::move(sch), factory_factory, options, schema_store_ptr, resolve_funcs)
        {
            init();
        }
        ...
        void init()
        {
            ...
            if (this->options().require_format_validation())
            {
                keyword_factory_map_.emplace("format", 
                    [&](const compilation_context<Json>& context, const Json& sch, const Json& parent, anchor_uri_map_type&){return this->make_format_validator(context, sch, parent);});
            }
#if defined(JSONCONS_HAS_STD_REGEX)
            ...
            keyword_factory_map_.emplace("propertyNames", 
                [&](const compilation_context<Json>& context, const Json& sch, const Json& parent, anchor_uri_map_type& anchor_dict){return this->make_property_names_validator(context, sch, parent, anchor_dict);});
            ...
            const Json& sch, anchor_uri_map_type& anchor_dict)
        {
            ...
            if (it != sch.object_range().end())
            {
                pattern_properties = this->make_pattern_properties_validator(context, (*it).value(), sch, anchor_dict);
            }
    #endif
