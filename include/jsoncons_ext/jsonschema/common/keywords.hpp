...
namespace jsonschema {
    ...
    public:
        ref_validator(const uri& base_uri) 
            : keyword_validator_base<Json>("$ref", base_uri)
        {}

        ref_validator(const uri& base_uri, schema_validator_type&& target)
            : keyword_validator_base<Json>("$ref", base_uri), referred_schema_(std::move(target)) {}
    ...
    public:
        recursive_ref_validator(const uri& base_uri) 
            : keyword_validator_base<Json>("$recursiveRef", base_uri), referred_schema_(nullptr)
        {}

        recursive_ref_validator(const uri& base_uri, schema_validator_type&& target)
            : keyword_validator_base<Json>("$recursiveRef", base_uri), referred_schema_(std::move(target)) {}
        ...
        keyword_validator_type clone() const override 
        {
            ...
            return jsoncons::make_unique<recursive_ref_validator>(this->schema_path(), referred_schema_ ? referred_schema_->clone() : nullptr);
        }
    ...
    private:
            ...
            Json&) const final
        {
            if (format_check_ != nullptr) 
            {
                ...
                format_check_(eval_path, this->schema_path(), instance_location, s, reporter);
    ...
    private:
            ...
            Json& patch) const final
        {
            ...
            for (const auto& item : instance.array_range()) 
            {
                ...
                {
                    pointer /= index++;
                    (*validator_it)->validate(eval_path, item, pointer, evaluated_properties, reporter, patch);
                    ++validator_it;
                }
                else if (additional_items_validator_ != nullptr)
                {
                    pointer /= index++;
                    additional_items_validator_->validate(eval_path, item, pointer, evaluated_properties, reporter, patch);
                }
                else
                    break;
            ...
            Json& patch) const final
        {
            ...
            if (validator_) 
            {
                ...
                for (const auto& item : instance.array_range()) 
                {
                    ...
                    validator_->validate(eval_path, item, instance_location, evaluated_properties, local_reporter, patch);
            ...
            Json& patch) const final
        {
            ...
            if (items_validator_)
            {
                for (const auto& i : instance.array_range()) 
                {
                    jsonpointer::json_pointer pointer(instance_location);
                    pointer /= index;
                    items_validator_->validate(eval_path, i, pointer, evaluated_properties, reporter, patch);
                    index++;
                }
            }
        }
    ...
    public:
        string_validator(const uri& schema_path,
            ...
            : keyword_validator_base<Json>("string", schema_path), validators_(std::move(validators))
        {
        }
        ...
        keyword_validator_type clone() const final 
        {
            ...
            return jsoncons::make_unique<string_validator>(this->schema_path(),
                std::move(validators));
        }
    ...
    private:
            ...
            Json& patch) const final
        {
            ...
            {
                validator->validate(eval_path, instance, instance_location, evaluated_properties, reporter, patch);
                if (reporter.error_count() > 0 && reporter.fail_early())
                {
                    return;
                }
    ...
    private:
        ...
        {
            collecting_error_reporter local_reporter;
            rule_->validate(eval_path, instance, instance_location, evaluated_properties, local_reporter, patch);
    ...
    private:
            ...
            Json& patch) const final
        {
            ...
            {
                std::size_t mark = local_reporter.errors.size();
                s->validate(eval_path, instance, instance_location, evaluated_properties, local_reporter, patch);
                if (!is_complete)
                {
                    if (mark == local_reporter.errors.size())
                        count++;
                    if (Criterion::is_complete(eval_path, instance, instance_location, reporter, local_reporter, count))
                        is_complete = true;
                }
            }
    ...
    public:
        ...
        keyword_validator_type clone() const final 
        {
            //std::cout << "clone minimum " << this->schema_path().string() << ", resolve: " << this->schema_path().string() << "\n\n"; 
    ...
    public:
        integer_validator(const uri& schema_path, 
            std::vector<keyword_validator_type>&& validators)
            : keyword_validator_base<Json>("integer", schema_path), validators_(std::move(validators))
        {
        }
        ...
        keyword_validator_type clone() const final 
        {
            ...
            return jsoncons::make_unique<integer_validator>(this->schema_path(),
                std::move(validators));
        }
    ...
    public:
        number_validator(const uri& schema_path, 
            std::vector<keyword_validator_type>&& validators)
            : keyword_validator_base<Json>("number", schema_path), validators_(std::move(validators))
        {
        }
        ...
        keyword_validator_type clone() const final 
        {
            ...
            return jsoncons::make_unique<number_validator>(this->schema_path(),
                std::move(validators));
        }
    ...
    public:
        null_validator(const uri& schema_path)
            : keyword_validator_base<Json>("null", schema_path)
        {
        }

        keyword_validator_type clone() const final 
        {
            return jsoncons::make_unique<null_validator>(this->schema_path());
        }
    ...
    public:
        boolean_validator(const uri& schema_path)
            : keyword_validator_base<Json>("boolean", schema_path)
        {
        }

        keyword_validator_type clone() const final 
        {
            return jsoncons::make_unique<boolean_validator>(this->schema_path());
        }
    ...
    private:
        ...
        void do_validate(const jsonpointer::json_pointer&, const Json&, 
            const jsonpointer::json_pointer&,
            std::unordered_set<std::string>&, 
            error_reporter&, 
            Json&) const final
    ...
    public:
        object_validator(const uri& schema_path,
            ...
            : keyword_validator_base<Json>("object", std::move(schema_path)), 
        ...
        keyword_validator_type clone() const final 
        {
            ...
            return jsoncons::make_unique<object_validator>(this->schema_path(),
    ...
    private:
            ...
            Json& patch) const final
        {
            ...
            for (const auto& prop : instance.object_range()) 
            {
                ...
                if (property_name_validator_)
                    property_name_validator_->validate(eval_path, prop.key() , instance_location, local_evaluated_properties, reporter, patch);
                ...
                if (properties_it != properties_.end()) 
                {
                    ...
                    properties_it->second->validate(eval_path, prop.value() , pointer, local_evaluated_properties, reporter, patch);
                ...
                for (auto& schema_pp : pattern_properties_)
                    if (std::regex_search(prop.key(), schema_pp.first)) 
                    {
                        a_prop_or_pattern_matched = true;
                        std::size_t error_count = reporter.error_count();
                        schema_pp.second->validate(eval_path, prop.value() , pointer, local_evaluated_properties, reporter, patch);
                        if (reporter.error_count() == error_count)
                        {
                            local_evaluated_properties.insert(prop.key());
                        }
                ...
                if (!a_prop_or_pattern_matched && additional_properties_) 
                {
                    ...
                    additional_properties_->validate(eval_path, prop.value() , pointer, local_evaluated_properties, local_reporter, patch);
                    if (!local_reporter.errors.empty())
                    {
                        reporter.error(validation_output(this->keyword_name(),
                            eval_path, 
            ...
            for (const auto& dep : dependent_required_) 
            {
                ...
                if (prop != instance.object_range().end()) 
                {
                    ...
                    jsonpointer::json_pointer pointer(instance_location);
                    pointer /= dep.first;
                    dep.second->validate(eval_path, instance, pointer, local_evaluated_properties, reporter, patch); // validate
                }
            }
            ...
            if (validator_)
            {
                for (const auto& prop : instance.object_range()) 
                {
                    ...
                    if (prop_it == evaluated_properties.end()) 
                    {
                        ...
                        std::size_t error_count = reporter.error_count();
                        validator_->validate(eval_path, prop.value() , instance_location, evaluated_properties, reporter, patch);
                        if (reporter.error_count() == error_count)
                        {
    ...
    public:
        array_validator(const uri& schema_path, std::vector<keyword_validator_type>&& validators)
            : keyword_validator_base<Json>("array", schema_path), validators_(std::move(validators))
        {
        }
        ...
        keyword_validator_type clone() const final 
        {
            ...
            return jsoncons::make_unique<array_validator>(this->schema_path(),
                std::move(validators));
        }
    ...
    private:
            ...
            Json& patch) const final
        {
            if (if_validator_) 
            {
                ...
                if_validator_->validate(eval_path, instance, instance_location, evaluated_properties, local_reporter, patch);
                if (local_reporter.errors.empty()) 
                {
                    if (then_validator_)
                        then_validator_->validate(eval_path, instance, instance_location, evaluated_properties, reporter, patch);
                } 
                else 
                {
                    if (else_validator_)
                        else_validator_->validate(eval_path, instance, instance_location, evaluated_properties, reporter, patch);
                }
            }
        }
    };
    ...
    public:
        enum_validator(const uri& path, const Json& sch)
            : keyword_validator_base<Json>("enum", path), value_(sch)
        {
        }
    ...
    public:
        const_validator(const uri& path, const Json& sch)
            : keyword_validator_base<Json>("const", path), value_(sch)
        {
        }
    ...
    private:
            ...
            Json& patch) const final
        {
            ...
            if (type)
                type->validate(eval_path, instance, instance_location, evaluated_properties, reporter, patch);
            else
            {
