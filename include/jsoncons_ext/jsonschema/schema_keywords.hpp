...
namespace jsonschema {
    ...
    {
        std::vector<validation_event> errors;
                                  ...
                                  error_reporter& reporter)
    {
        if (content_media_type == "application/Json")
        {
            ...
            if (ec)
            {
                reporter.error(validation_event(instance_location.string(), std::string("Content is not JSON: ") + ec.message(), "contentMediaType", "foo"));
            }
        }
    }
                         ...
                         Json&) const override
        {
            ...
            if (content_encoding_)
            {
                if (*content_encoding_ == "base64")
                {
                    ...
                    {
                        reporter.error(validation_event(instance_location.string(), "Content is not a base64 string", "contentEncoding", this->absolute_keyword_location()));
                    }
                }
                else if (!content_encoding_->empty())
                {
                    reporter.error(validation_event(instance_location.string(), "unable to check for contentEncoding '" + *content_encoding_ + "'", "contentEncoding", this->absolute_keyword_location()));
                }
            }
            else
            {
            ...
            {
                reporter.error(validation_event(instance_location.string(), "Expected string, but is byte string", "contentMediaType", this->absolute_keyword_location()));
            }
            ...
            if (min_length_) 
            {
                ...
                {
                    reporter.error(validation_event(instance_location.string(), std::string("Expected minLength: ") + std::to_string(*min_length_)
            ...
            if (max_length_) 
            {
                ...
                {
                    reporter.error(validation_event(instance_location.string(), std::string("Expected maxLength: ") + std::to_string(*max_length_)
            ...
            if (pattern_)
            {
                if (!std::regex_search(content, *pattern_))
                {
                    ...
                    message.append("\"");
                    reporter.error(validation_event(instance_location.string(), message, "pattern", this->absolute_keyword_location()));
                }
            }
    ...
    private:
                         ...
                         Json& patch) const final
        {
            ...
            if (local_reporter.errors.empty())
                reporter.error(validation_event(instance_location.string(), "Instance must not be valid against schema", "not", this->absolute_keyword_location()));
        }
    ...
    struct one_of_criterion
    {
                                ...
                                std::size_t count)
        {
            if (count > 1)
            {
                ...
                reporter.error(validation_event(instance_location.string(), message, "oneOf", "foo"));
            }
            return count > 1;
        }
    };
    ...
    private:
                         ...
                         Json&) const override
        {
            ...
            {
                reporter.error(validation_event(instance_location.string(), "Instance is not a number", "number", this->absolute_keyword_location()));
            }

            if (multiple_of_ && value != 0) // zero is multiple of everything
                if (violates_multiple_of(value))
                {
                    reporter.error(validation_event(instance_location.string(), instance.template as<std::string>() + " is not a multiple of " + std::to_string(*multiple_of_), "multipleOf", this->absolute_keyword_location()));
                }

            if (maximum_)
                if ((exclusive_maximum_ && value >= *maximum_) ||
                    value > *maximum_)
                    reporter.error(validation_event(instance_location.string(), instance.template as<std::string>() + " exceeds maximum of " + std::to_string(*maximum_), "maximum", this->absolute_keyword_location()));

            if (minimum_)
                if ((exclusive_minimum_ && value <= *minimum_) ||
                    value < *minimum_)
                    reporter.error(validation_event(instance_location.string(), instance.template as<std::string>() + " is below minimum of " + std::to_string(*minimum_), "minimum", this->absolute_keyword_location()));
        }
    ...
    private:
        void do_validate(const uri_wrapper& instance_location, const Json& instance, error_reporter& reporter, Json&) const override
        {
            if (!instance.is_null())
            {
                reporter.error(validation_event(instance_location.string(), "Expected to be null", "null", this->absolute_keyword_location()));
            }
        }
    };
    ...
    private:
        ...
        {
            reporter.error(validation_event(instance_location.string(), "False schema always fails", "false", this->absolute_keyword_location()));
        }
    };
    ...
    private:
        ...
        void do_validate(const uri_wrapper& instance_location, const Json& instance, error_reporter& reporter, Json&) const override final
        {
            for (const auto& key : required_)
            {
                ...
                {
                    reporter.error(validation_event(instance_location.string(), "Required property \"" + key + "\" not found", "required", this->absolute_keyword_location()));
                }
            }
        }
    };
    ...
    private:
                         ...
                         Json& patch) const override
        {
            if (max_properties_ && instance.size() > *max_properties_)
            {
                ...
                reporter.error(validation_event(instance_location.string(), message, "maxProperties", this->absolute_keyword_location()));
            }
            ...
            if (min_properties_ && instance.size() < *min_properties_)
            {
                ...
                message.append(", found: " + std::to_string(instance.size()));
                reporter.error(validation_event(instance_location.string(), message, "minProperties", this->absolute_keyword_location()));
            }
            ...
            for (const auto& property : instance.object_range()) 
            {
                ...
                if (!a_prop_or_pattern_matched && additional_properties_) 
                {
                    ...
                    if (!local_reporter.errors.empty())
                        reporter.error(validation_event(instance_location.string(), "Additional property \"" + property.key() + "\" found but was invalid.", "additionalProperties", additional_properties_->absolute_keyword_location()));
                }
            }
    ...
    private:
                         ...
                         Json& patch) const override
        {
            if (max_items_ && instance.size() > *max_items_)
            {
                ...
                message.append(", found: " + std::to_string(instance.size()));
                reporter.error(validation_event(instance_location.string(), message, "maxItems", this->absolute_keyword_location()));
            }
            ...
            if (min_items_ && instance.size() < *min_items_)
            {
                ...
                reporter.error(validation_event(instance_location.string(), message, "minItems", this->absolute_keyword_location()));
            }
            ...
            if (unique_items_) 
            {
                for (auto it = instance.object_range().cbegin(); it != instance.object_range().cend(); ++it) 
                {
                    ...
                    if (v != instance.object_range().end())
                        reporter.error(validation_event(instance_location.string(), "Array items are not unique", "uniqueItems", this->absolute_keyword_location()));
                }
            }
            ...
            if (contains_) 
            {
                ...
                if (!contained)
                    reporter.error(validation_event(instance_location.string(), "Expected at least one array item to match \"contains\" schema", "contains", this->absolute_keyword_location(), local_reporter.errors));
            }
        }
    };
    ...
    private:
                         ...
                         Json&) const final
        {
            ...
            if (!in_range)
            {
                reporter.error(validation_event(instance_location.string(), instance.template as<std::string>() + " is not a valid enum value", "enum", this->absolute_keyword_location()));
            }
        }
    };
    ...
    private:
        ...
        {
            if (const_ != instance)
                reporter.error(validation_event(instance_location.string(), "Instance is not const", "const", this->absolute_keyword_location()));
        }
    };
    ...
    private:
                         ...
                         Json& patch) const override final
        {
            ...
            else
            {
                ...
                reporter.error(validation_event(instance_location.string(), ss.str(), "type", this->absolute_keyword_location()));
            }
