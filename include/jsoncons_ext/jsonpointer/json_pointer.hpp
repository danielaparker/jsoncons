...
#ifndef JSONCONS_EXT_JSONPOINTER_JSON_POINTER_HPP
#define JSONCONS_EXT_JSONPOINTER_JSON_POINTER_HPP
...
public:
    ...
    basic_json_pointer(const allocator_type&)
    {
    }
    ...
    basic_json_pointer(string_view_type s, std::error_code& ec)
    {
        auto jp = parse(s, ec);
        if (!ec)
        {
    ...
    basic_json_pointer(const basic_json_pointer& other) = default;

    basic_json_pointer(basic_json_pointer&& other) noexcept = default;

    basic_json_pointer& operator=(const basic_json_pointer& other) = default;

    basic_json_pointer& operator=(basic_json_pointer&& other) noexcept = default;
    ...
    static basic_json_pointer parse(std::allocator_arg_t, const allocator_type& alloc, string_view_type input, std::error_code& ec)
    {
        ...
        while (p < pend)
        {
                switch (state)
                {
                    ...
                    case jsonpointer::detail::pointer_state::new_token: 
                        ...
                        {
                            case '/':
                                tokens.emplace_back(pos, buffer.size()-pos);
                                pos = buffer.size();
        ...
        return basic_json_pointer<char_type,allocator_type>{std::move(buffer), std::move(tokens)};
    }
