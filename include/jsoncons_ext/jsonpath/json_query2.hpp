...
public:
                  ...
                  std::error_code& ec)
    {
        ...
        while (p_ < end_input_)
        {
            switch (state_stack_.back().state)
            {
                ...
                case path_state::path_or_function_name:
                    switch (*p_)
                    {
                        ...
                        case '.':
                        {
                            ...
                            state_stack_.back().state = path_state::dot2;
                            break;
                        }
                        case ' ':case '\t':
                        {
                ...
                case path_state::dot:
                    switch (*p_)
                    {
                        ...
                        case '.':
                            apply_selectors();
                            buffer.clear();
                            state_stack_.back() = path_state::dot2;
                            break;
                        case '[':
                            apply_selectors();
                            buffer.clear();
                ...
                case path_state::name_or_left_bracket: // Can [ follow .?
                    switch (*p_)
                    {
                        ...
                        default:
                            ...
                            state_stack_.emplace_back(path_state::unquoted_name, state_stack_.back());
                            break;
                    }
                    break;
                case path_state::dot_or_left_bracket: 
                    switch (*p_)
                    {
                        case ' ':case '\t':
                            break;
                        case '.':
                            state_stack_.emplace_back(path_state::dot2, state_stack_.back());
                            break;
                        case '[':
                ...
                case path_state::expr_or_filter_or_slice_or_key:
                    ...
                    break;
                case path_state::unquoted_name_in_brackets:
                    switch (*p_)
                    {
                        case ':':
        ...
        {
            std::cout << "stack: ";
            for (auto st : state_stack_)
            {
                std::cout << (int)st.state << " "; 
            }
            std::cout << "\n\n";
