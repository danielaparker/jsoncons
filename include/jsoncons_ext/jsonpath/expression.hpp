...
namespace detail {
    ...
    struct path_value_pair
    {
        ...
        using path_pointer = const path_component_type*;
    ...
    struct path_component_value_pair
    {
        ...
        using path_pointer = const path_component_type*;
    private:
        const path_component_type* last_ptr_;
        value_pointer value_ptr_;
    public:
        path_component_value_pair(const path_component_type& last, reference value) noexcept
    ...
    public:
        ...
        virtual void add_node(const path_component_type& path_tail, 
                                reference value) = 0;
    };
    ...
    public:
        ...
        void add_node(const path_component_type& path_tail, 
    ...
    class dynamic_resources
    {
        using reference = JsonReference;
        using pointer = typename std::conditional<std::is_const<typename std::remove_reference<reference>::type>::value,typename Json::const_pointer,typename Json::pointer>::type;
        using path_component_type = path_component<typename Json::char_type>;
        using path_stem_value_pair_type = path_component_value_pair<Json,JsonReference>;
        std::vector<std::unique_ptr<Json>> temp_json_values_;
        std::vector<std::unique_ptr<path_component_type>> temp_path_node_values_;
    ...
    public:
        ...
        const path_component_type& root_path_node() const
        {
            static path_component_type root('$');
            return root;
        }

        const path_component_type& current_path_node() const
        {
            static path_component_type root('@');
            return root;
        }
        ...
        template <typename... Args>
        const path_component_type* create_path_node(Args&& ... args)
        {
            auto temp = jsoncons::make_unique<path_component_type>(std::forward<Args>(args)...);
            path_component_type* ptr = temp.get();
    ...
    public:
        ...
        virtual void select(dynamic_resources<Json,JsonReference>& resources,
                            reference root,
                            const path_component_type& path_tail, 
                            reference val, 
