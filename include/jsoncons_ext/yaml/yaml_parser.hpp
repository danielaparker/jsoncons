...
#ifndef JSONCONS_YAML_YAML_PARSER_HPP
#define JSONCONS_YAML_YAML_PARSER_HPP
...
#include <jsoncons_ext/yaml/yaml_error.hpp>
#include <jsoncons_ext/yaml/yaml_options.hpp>
...
namespace yaml {
    ...
    {
        bool operator()(yaml_errc ec, const ser_context&) noexcept 
        {
            if (ec == yaml_errc::illegal_comment)
            {
    ...
    {
        bool operator()(yaml_errc, const ser_context&) noexcept
        {
            return false;
        }
    };
    ...
    private:
        ...
        basic_yaml_parser(const basic_yaml_parser&) = delete;
        basic_yaml_parser& operator=(const basic_yaml_parser&) = delete;
    ...
    public:
        ...
        void begin_object(basic_json_visitor<CharT>& visitor, std::error_code& ec)
        {
            ...
            {
                more_ = err_handler_(yaml_errc::max_nesting_depth_exceeded, *this);
                if (!more_)
                {
                    ec = yaml_errc::max_nesting_depth_exceeded;
                    return;
                }
            } 
        ...
        void end_object(basic_json_visitor<CharT>& visitor, std::error_code& ec)
        {
            ...
            {
                err_handler_(yaml_errc::expected_comma_or_right_bracket, *this);
                ec = yaml_errc::expected_comma_or_right_bracket;
                more_ = false;
                return;
            }
            else
        ...
        void end_array(basic_json_visitor<CharT>& visitor, std::error_code& ec)
        {
            ...
            {
                err_handler_(yaml_errc::expected_comma_or_right_brace, *this);
                ec = yaml_errc::expected_comma_or_right_brace;
                more_ = false;
                return;
            }
            else
            ...
            for (; input_ptr_ != input_end_; ++input_ptr_)
            {
                ...
                switch (curr_char_)
                {
                    ...
                    case ' ':
                        break;
                    default:
                        more_ = err_handler_(yaml_errc::extra_character, *this);
                        if (!more_)
                        {
                            ec = yaml_errc::extra_character;
                            return;
                        }
                        break;
                }
            ...
            if (input_ptr_ == local_input_end && more_)
            {
                switch (state_)
                {
                    ...
                    case json_parse_state::cr:
                        ...
                        break;
                    default:
                        err_handler_(yaml_errc::unexpected_eof, *this);
                        ec = yaml_errc::unexpected_eof;
                        more_ = false;
                        return;
                }
            }
                ...
                switch (state_)
                {
                    ...
                    case json_parse_state::expect_comma_or_end: 
                        {
                            switch (*input_ptr_)
                            {
                                ...
                                default:
                                    ...
                                    {
                                        more_ = err_handler_(yaml_errc::expected_comma_or_right_bracket, *this);
                                        if (!more_)
                                        {
                                            ec = yaml_errc::expected_comma_or_right_bracket;
                                            return;
                                    ...
                                    {
                                        more_ = err_handler_(yaml_errc::expected_comma_or_right_brace, *this);
                                        if (!more_)
                                        {
                                            ec = yaml_errc::expected_comma_or_right_brace;
                                            return;
                    ...
                    case json_parse_state::expect_member_name_or_end: 
                        {
                            switch (*input_ptr_)
                            {
                                ...
                                case '\"':
                                    ...
                                    break;
                                case '\'':
                                    more_ = err_handler_(yaml_errc::single_quote, *this);
                                    if (!more_)
                                    {
                                        ec = yaml_errc::single_quote;
                                        return;
                                    }
                                    ++input_ptr_;
                                    ++position_;
                                    break;
                                default:
                                    more_ = err_handler_(yaml_errc::expected_key, *this);
                                    if (!more_)
                                    {
                                        ec = yaml_errc::expected_key;
                                        return;
                                    }
                                    ++input_ptr_;
                                    ++position_;
                    ...
                    case json_parse_state::expect_member_name: 
                        {
                            switch (*input_ptr_)
                            {
                                ...
                                case '\"':
                                    ...
                                    break;
                                case '}':
                                    more_ = err_handler_(yaml_errc::extra_comma, *this);
                                    if (!more_)
                                    {
                                        ec = yaml_errc::extra_comma;
                                        return;
                                    }
                    ...
                    case json_parse_state::expect_colon: 
                        {
                            switch (*input_ptr_)
                            {
                                ...
                                case ':':
                                    ...
                                    break;
                                default:
                                    more_ = err_handler_(yaml_errc::expected_colon, *this);
                                    if (!more_)
                                    {
                                        ec = yaml_errc::expected_colon;
                                        return;
                                    }
                                    ++input_ptr_;
                                    ++position_;
                        ...
                        case json_parse_state::expect_value: 
                        {
                            switch (*input_ptr_)
                            {
                                ...
                                case ']':
                                    ...
                                    {
                                        more_ = err_handler_(yaml_errc::expected_value, *this);
                                        if (!more_)
                                        {
                                            ec = yaml_errc::expected_value;
                                            return;
                                        }
                                    }
                                    ++input_ptr_;
                    ...
                    case json_parse_state::t: 
                        switch (*input_ptr_)
                        {
                            case 'r':
                                ...
                                break;
                            default:
                                err_handler_(yaml_errc::invalid_value, *this);
                                ec = yaml_errc::invalid_value;
                                more_ = false;
                                return;
                        }
                        break;
                    ...
                    case json_parse_state::slash: 
                    {
                        ...
                        case '*':
                            ...
                            more_ = err_handler_(yaml_errc::illegal_comment, *this);
                            if (!more_)
                            {
                                ec = yaml_errc::illegal_comment;
                                return;
                            }
                            break;
                        case '/':
                            ...
                            break;
                        default:    
                            more_ = err_handler_(yaml_errc::syntax_error, *this);
                            if (!more_)
                            {
                                ec = yaml_errc::syntax_error;
                                return;
    ...
    minus_sign:
            ...
            switch (*input_ptr_)
            {
                ...
                case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8': case '9':
                    ...
                    goto integer;
                default:
                    err_handler_(yaml_errc::expected_value, *this);
                    ec = yaml_errc::expected_value;
                    more_ = false;
                    return;
            }
    zero:
            ...
            switch (*input_ptr_)
            {
                ...
                case '0': case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8': case '9':
                    err_handler_(yaml_errc::leading_zero, *this);
                    ec = yaml_errc::leading_zero;
                    more_ = false;
                    state_ = json_parse_state::zero;
                    return;
                default:
                    err_handler_(yaml_errc::invalid_number, *this);
                    ec = yaml_errc::invalid_number;
                    more_ = false;
                    state_ = json_parse_state::zero;
                    return;
            }
    ...
    string_u1:
            while (input_ptr_ < local_input_end)
            {
                switch (*input_ptr_)
                {
                    ...
                    {
                        position_ += (input_ptr_ - sb + 1);
                        more_ = err_handler_(yaml_errc::illegal_character_in_string, *this);
                        if (!more_)
                        {
                            ec = yaml_errc::illegal_character_in_string;
                            state_ = json_parse_state::string;
                            return;
                        }
                        // recovery - keep
    ...
    escape:
            ...
            case 'u':
                 ...
                 goto escape_u1;
            default:    
                err_handler_(yaml_errc::illegal_escaped_character, *this);
                ec = yaml_errc::illegal_escaped_character;
                more_ = false;
                state_ = json_parse_state::escape;
                return;
            }
    ...
    escape_expect_surrogate_pair1:
            ...
            }
            {
                ...
                case '\\': 
                    ...
                    goto escape_expect_surrogate_pair2;
                default:
                    err_handler_(yaml_errc::expected_codepoint_surrogate_pair, *this);
                    ec = yaml_errc::expected_codepoint_surrogate_pair;
                    more_ = false;
                    state_ = json_parse_state::escape_expect_surrogate_pair1;
                    return;
                }
    ...
    escape_u8:
        ...
        void translate_conv_errc(unicons::conv_errc result, std::error_code& ec)
        {
            ...
            case unicons::conv_errc::over_long_utf8_sequence:
                more_ = err_handler_(yaml_errc::over_long_utf8_sequence, *this);
                if (!more_)
                {
                    ec = yaml_errc::over_long_utf8_sequence;
                    return;
                }
                break;
            case unicons::conv_errc::unpaired_high_surrogate:
                more_ = err_handler_(yaml_errc::unpaired_high_surrogate, *this);
                if (!more_)
                {
                    ec = yaml_errc::unpaired_high_surrogate;
                    return;
                }
                break;
            case unicons::conv_errc::expected_continuation_byte:
                more_ = err_handler_(yaml_errc::expected_continuation_byte, *this);
                if (!more_)
                {
                    ec = yaml_errc::expected_continuation_byte;
                    return;
                }
                break;
            case unicons::conv_errc::illegal_surrogate_value:
                more_ = err_handler_(yaml_errc::illegal_surrogate_value, *this);
                if (!more_)
                {
                    ec = yaml_errc::illegal_surrogate_value;
                    return;
                }
                break;
            default:
                more_ = err_handler_(yaml_errc::illegal_codepoint, *this);
                if (!more_)
                {
                    ec = yaml_errc::illegal_codepoint;
                    return;
                }
                break;
            }
    ...
    private:
        ...
        void end_fraction_value(basic_json_visitor<CharT>& visitor, std::error_code& ec)
        {
            ...
            JSONCONS_CATCH(...)
            {
                more_ = err_handler_(yaml_errc::invalid_number, *this);
                if (!more_)
                {
                    ec = yaml_errc::invalid_number;
                    return;
        ...
        uint32_t append_to_codepoint(uint32_t cp, int c, std::error_code& ec)
        {
            ...
            {
                more_ = err_handler_(yaml_errc::invalid_unicode_escape_sequence, *this);
                if (!more_)
                {
                    ec = yaml_errc::invalid_unicode_escape_sequence;
                    return cp;
                }
            }
            return cp;
