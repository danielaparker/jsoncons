#ifndef JSONCONS_FLAT_HASH_MAP_HPP
#define JSONCONS_FLAT_HASH_MAP_HPP
...
#else
#define JSONCONS_NOINLINE2(...) __VA_ARGS__ __attribute__((noinline))
#endif
...
public:
        ...
        : EntryAlloc(alloc), Hasher(other), Equal(other), _max_load_factor(other._max_load_factor)
    {
        ...
        {
            clear();
            deallocate_data(entries, num_slots_minus_one_, max_lookups_);
            throw;
        }
    }
    ...
    iterator end()
    {
        return { entries + static_cast<ptrdiff_t>(num_slots_minus_one_ + max_lookups_) };
    }
    ...
    void rehash(size_t num_buckets)
    {
        ...
        int8_t old_max_lookups = max_lookups_;
    ...
    void clear()
    {
        for (EntryPointer it = entries, end = it + static_cast<ptrdiff_t>(num_slots_minus_one_ + max_lookups_); it != end; ++it)
        {
            if (it->has_value())
                it->destroy_value();
        }
    ...
    {
        return hash_policy.index_for_hash(hash_object(key), num_slots_minus_one_);
    }
...
private:
    ...
    template<typename Key, typename... Args>
    JSONCONS_NOINLINE2(std::pair<iterator, bool>) emplace_new_key(int8_t distance_from_desired, EntryPointer current_entry, Key && key, Args &&... args)
    {
        using std::swap;
        if (num_slots_minus_one_ == 0 || distance_from_desired == max_lookups_ || num_elements + 1 > (num_slots_minus_one_ + 1) * static_cast<double>(_max_load_factor))
        {
            grow();
...
struct fibonacci_hash_policy
{
    size_t index_for_hash(size_t hash, size_t /*num_slots_minus_one*/) const
    {
        return (11400714819323198485ull * hash) >> shift_;
    }
    size_t keep_in_range(size_t index, size_t num_slots_minus_one) const
    {
...
{
    typedef jsoncons::power_of_two_hash_policy hash_policy;
};

} // end namespace jsoncons

#endif // JSONCONS_FLAT_HASH_MAP_HPP
