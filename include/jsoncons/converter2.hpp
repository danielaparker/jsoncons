...
namespace jsoncons {
    ...
    class converter<Into, 
        typename std::enable_if<type_traits::is_basic_string_view<Into>::value &&
        type_traits::is_basic_string<Into>::value>::type>
    {
    public:
        Into convert(From value)
        {
            return Into(value.data(), value.size());
        }
    };
    ...
    template <class Into>
    class converter2<Into,typename std::enable_if<(!type_traits::is_basic_string<Into>::value && 
    ...
    // Into string
    template <class Into>
    class converter2<Into,typename std::enable_if<type_traits::is_basic_string<Into>::value>::type>
    {
        using char_type = typename Into::value_type;
        using allocator_type = typename Into::allocator_type;
        int dummy_;
    public:
        explicit converter2(int dummy = int())
            : dummy_(dummy)
        {}
    ...
    private:
        ...
        {
            converter2<std::string> convert{ dummy_ };
