...
namespace jsoncons {
    ...
    public:
            ...
            : cursor_ptr_(std::addressof(cursor)), done_(false)
        {
            ...
            else if (cursor_ptr_->current().event_type() == staj_event_types::begin_array)
            {
                next();
            }
            else
    ...
    private:
        ...
        void next(std::error_code& ec)
        {
            ...
            if (JSONCONS_UNLIKELY(cursor_ptr_->current().event_type() == staj_event_types::end_array))
            {
                done_ = true;
                return;
            }
    ...
    public:
            ...
            : cursor_ptr_(std::addressof(cursor)), done_(false)
        {
            ...
            else if (cursor_ptr_->current().event_type() == staj_event_types::begin_object)
            {
                next();
            }
            else
    ...
    private:
        ...
        void next(std::error_code& ec)
        {
            ...
            if (JSONCONS_UNLIKELY(cursor_ptr_->current().event_type() == staj_event_types::end_object))
            {
                done_ = true;
                return;
            }
            JSONCONS_ASSERT(cursor_ptr_->current().event_type() == staj_event_types::key);
