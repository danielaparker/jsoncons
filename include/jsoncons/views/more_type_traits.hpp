...
namespace utility {
   ...
   is_detected_exact = std::is_same< Expected, is_detected_t<Op, Args...> >;
    ...
    template <typename T>
    struct is_byte<T, 
           typename std::enable_if<std::is_same<std::remove_cv_t<T>,char>::value ||
                                   std::is_same<std::remove_cv_t<T>,signed char>::value ||
                                   std::is_same<std::remove_cv_t<T>,unsigned char>::value ||
                                   is_std_byte<std::remove_cv_t<T>>::value
    >::type> : std::true_type {};
    ...
    template <typename Container>
    using
    container_emplace_back_t = decltype(std::declval<Container>().emplace_back(std::declval<typename Container::value_type>()));
    ...
    template <class Container, typename Enable=void>
    ...
    template <class Container, typename ValueT, typename Enable=void>
    struct is_sequence_of : std::false_type {};

    template <class Container, typename ValueT>
...
namespace impl {
    ...
    template <class Source, typename Enable=void>
