...
1 (epoch time)  
CBOR epoch times are decoded into jsoncons int64_t, uint64_t and double and tagged with `semantic_tag::epoch_seconds`. 
jsoncons int64_t, uint64_t and double tagged with `semantic_tag::epoch_seconds` are encoded into CBOR epoch time.
