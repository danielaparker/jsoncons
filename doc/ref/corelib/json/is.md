...
(1) Generic `is` equivalent to type `T`. Returns **true** if the json value is the same as type `T` according to [json_traits](../corelib/reflect/json_traits.md), **false** otherwise.  

    bool is<X<T>> const noexcept
If the type `X` is not `std::basic_string` but otherwise satisfies [SequenceContainer](http://en.cppreference.com/w/cpp/concept/SequenceContainer), `is<X<T>>()` returns **true** if the json value is an array and each element is the "same as" type `T` according to [json_traits](./reflect/json_traits.md), **false** otherwise.
    bool is<X<std::string,T>> const noexcept
If the type 'X' satisfies [AssociativeContainer](http://en.cppreference.com/w/cpp/concept/AssociativeContainer) or [UnorderedAssociativeContainer](http://en.cppreference.com/w/cpp/concept/UnorderedAssociativeContainer), `is<X<T>>()` returns **true** if the json value is an object and each mapped value is the "same as" `T` according to [json_traits](./reflect/json_traits.md), **false** otherwise.
