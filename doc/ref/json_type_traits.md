...
`jsoncons::optional<T>` <sup>1</sup>|`true` if `j.is_null()` or `j` is convertible as `T`|`std::optional<T>()` if `j.is_null()`, otherwise `std::optional<T>(j.as<T>())`|<em>&#x2713;</em>

<sup>1</sup>. `jsoncons::optional<T>` is typedef'ed to [std::optional<T>](https://en.cppreference.com/w/cpp/utility/optional) if `JSONCONS_HAS_STD_OPTIONAL` is defined. 
